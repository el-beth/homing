				...
				while read dir
					do
						if [ -d "$dir" ]
							then
								dir="$(sed -E 's/^\.\///gi' <<< "$dir")";
								iterated="$iterated"'<a href="'"http://mov.ez/cgi-bin/moviesy_korean/loader_dl.sh?name=$dir&session=$session"'"><div class="card neu-passive"><div class="card-img neu_inset"><img src="'"http://mov.ez/korean/$dir/poster.jpg"'"></div><div class="card-button neu_inset"><span>'"$dir"'</span></div></div></a>';
						fi
				done <<< "$finds"
			else
				iterated="$iterated"'<center><h1 class="neu" style="margin: 10px; border-r"a>Nothing Found!</br>search again or click <a href="http://mov.ez/cgi-bin/moviesy_korean/main_dl.sh?session='"$session"'"><span	style="color: cyan;">here</span></a> to go back to the catalogue</h1></center>'
		fi
