					...
					do
						if [ -d "$dir" ]
							then
								...
								iterated="$iterated"'<a href="http://mov.ez/cgi-bin/moviesy_feature/loader.sh?/var/www/html/movie/'"$dir"'&session='"$session"'"><div class="card neu-passive"><div class="card-img neu_inset"><img src="'"http://mov.ez/movie/$dir/poster.jpg"'"></div><div class="card-button neu_inset"><span>'"$movie_name"'</span></div></div></a>';
						fi
				done <<< "$finds"
			else
				iterated="$iterated"'<center><h1 class="neu" style="margin: 10px; border-r"a>Nothing Found!</br>search again or click <a href="http://mov.ez/cgi-bin/moviesy_feature/main.sh?page=1&session='"$session"'"><span	style="color: cyan;">here</span></a> to go back to the catalogue</h1></center>'
		fi
